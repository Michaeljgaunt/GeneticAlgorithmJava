...
import GA.GUI.RankingMethod;
import GA.GUI.Optimisation;
...
public class Driver {
    ...
    public static void runBitStrings() {
        ...
        numIts = GUI.getNumIts();
        chromNum = GUI.getChromNum();
        varNum = GUI.getVarNum();
        lowerBound = GUI.getLowerBound();
        upperBound = GUI.getUpperBound();
        numCuts = GUI.getNumCuts();
        prMutRate = GUI.getPrMutRate();
        seMutRate = GUI.getSeMutRate();
        fitnessThreshold = GUI.getFitThreshold();
        tourneySize = GUI.getTourneySize();
        frFlag = GUI.isFullResultsSelected();
        dynamicMutFlag = GUI.isDynamicMutationSelected();
                
        //Setting the mutation changed flag.
        boolean mutChanged = false;

        //Setting the ranking method chosen by the user.
        RankingMethod rank = GUI.getRankButState();
        ...
        Optimisation optimisation = GUI.getOptimization();
        switch(optimisation) {
            case MINIMISE:
                maxFlag = false;
                minFlag = true;
